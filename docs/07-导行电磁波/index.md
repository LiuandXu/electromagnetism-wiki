...
!!! warning "真题"
    ...
    **真题 7.15（来源：2021秋季A卷 三、1）**
    **题目：** 已知横截面积为 $a\times b$ 的矩形金属波导中电磁场的复数形式为：

    $$\dot{\mathbf{E}}(x,z) = -\hat{\mathbf{y}}\,jE_0\sin\!\left(\frac{\pi x}{a}\right)e^{-j\beta z}$$

    $$\dot{\mathbf{H}}(x,z) = \left[\hat{\mathbf{x}}\,jH_{0x}\sin\!\left(\frac{\pi x}{a}\right) + \hat{\mathbf{z}}\,H_{0z}\cos\!\left(\frac{\pi x}{a}\right)\right]e^{-j\beta z}$$

    式中，$E_0$、$H_{0x}$、$H_{0z}$、$\beta$ 都是常数。试求：

    (1) 电磁场的瞬时形式；
    (2) 求瞬时坡印廷矢量 $\mathbf{S}$；求平均坡印廷矢量 $\mathbf{S}_{\mathrm{av}}$。
    ...
    ??? note "点击查看解答"
        ...
        **(1) 电磁场的瞬时形式**
        ...
        由 $\mathbf{E}(x,z,t) = \operatorname{Re}[\dot{\mathbf{E}}\,e^{j\omega t}]$：

        电场：

        $$\mathbf{E}(x,z,t) = \operatorname{Re}\!\left[-\hat{\mathbf{y}}\,jE_0\sin\!\left(\frac{\pi x}{a}\right)e^{j(\omega t - \beta z)}\right]$$

        由于 $-j = e^{-j\pi/2}$，故 $-j\cdot e^{j\theta} = e^{j(\theta - \pi/2)}$，取实部得 $\cos(\theta - \pi/2) = \sin\theta$：

        $$\mathbf{E}(x,z,t) = \hat{\mathbf{y}}\,E_0\sin\!\left(\frac{\pi x}{a}\right)\sin(\omega t - \beta z)\;\text{(V/m)}$$

        磁场：

        $$\mathbf{H}(x,z,t) = \operatorname{Re}\!\left\{\left[\hat{\mathbf{x}}\,jH_{0x}\sin\!\left(\frac{\pi x}{a}\right) + \hat{\mathbf{z}}\,H_{0z}\cos\!\left(\frac{\pi x}{a}\right)\right]e^{j(\omega t - \beta z)}\right\}$$

        $x$ 分量：$j\cdot e^{j\theta}$ 取实部 $= -\sin\theta$；$z$ 分量：$e^{j\theta}$ 取实部 $= \cos\theta$：

        $$\mathbf{H}(x,z,t) = -\hat{\mathbf{x}}\,H_{0x}\sin\!\left(\frac{\pi x}{a}\right)\sin(\omega t - \beta z) + \hat{\mathbf{z}}\,H_{0z}\cos\!\left(\frac{\pi x}{a}\right)\cos(\omega t - \beta z)\;\text{(A/m)}$$

        **(2) 坡印廷矢量**

        **瞬时坡印廷矢量** $\mathbf{S} = \mathbf{E}\times\mathbf{H}$：

        $$\mathbf{S} = \hat{\mathbf{y}}\,E_0\sin\!\left(\frac{\pi x}{a}\right)\sin(\omega t - \beta z)\times\left[-\hat{\mathbf{x}}\,H_{0x}\sin\!\left(\frac{\pi x}{a}\right)\sin(\omega t - \beta z) + \hat{\mathbf{z}}\,H_{0z}\cos\!\left(\frac{\pi x}{a}\right)\cos(\omega t - \beta z)\right]$$

        利用 $\hat{\mathbf{y}}\times(-\hat{\mathbf{x}}) = \hat{\mathbf{z}}$，$\hat{\mathbf{y}}\times\hat{\mathbf{z}} = \hat{\mathbf{x}}$：

        $$\mathbf{S} = \hat{\mathbf{z}}\,E_0 H_{0x}\sin^2\!\left(\frac{\pi x}{a}\right)\sin^2(\omega t - \beta z) + \hat{\mathbf{x}}\,E_0 H_{0z}\sin\!\left(\frac{\pi x}{a}\right)\cos\!\left(\frac{\pi x}{a}\right)\sin(\omega t - \beta z)\cos(\omega t - \beta z)$$

        利用倍角公式化简：

        $$\mathbf{S} = \hat{\mathbf{z}}\,E_0 H_{0x}\sin^2\!\left(\frac{\pi x}{a}\right)\sin^2(\omega t - \beta z) + \hat{\mathbf{x}}\,\frac{E_0 H_{0z}}{4}\sin\!\left(\frac{2\pi x}{a}\right)\sin 2(\omega t - \beta z)\;\text{(W/m}^2\text{)}$$

        **平均坡印廷矢量** $\mathbf{S}_{\mathrm{av}} = \dfrac{1}{2}\operatorname{Re}[\dot{\mathbf{E}}\times\dot{\mathbf{H}}^*]$：

        $$\dot{\mathbf{H}}^* = \left[-\hat{\mathbf{x}}\,jH_{0x}\sin\!\left(\frac{\pi x}{a}\right) + \hat{\mathbf{z}}\,H_{0z}\cos\!\left(\frac{\pi x}{a}\right)\right]e^{j\beta z}$$

        $$\dot{\mathbf{E}}\times\dot{\mathbf{H}}^* = -\hat{\mathbf{y}}\,jE_0\sin\!\left(\frac{\pi x}{a}\right)\left[-\hat{\mathbf{x}}\,jH_{0x}\sin\!\left(\frac{\pi x}{a}\right) + \hat{\mathbf{z}}\,H_{0z}\cos\!\left(\frac{\pi x}{a}\right)\right]$$

        $$= \hat{\mathbf{z}}\,E_0 H_{0x}\sin^2\!\left(\frac{\pi x}{a}\right) - \hat{\mathbf{x}}\,j\,\frac{E_0 H_{0z}}{2}\sin\!\left(\frac{2\pi x}{a}\right)$$

        $x$ 分量为纯虚数，取实部后为零；$z$ 分量为实数：

        $$\mathbf{S}_{\mathrm{av}} = \hat{\mathbf{z}}\,\frac{E_0 H_{0x}}{2}\sin^2\!\left(\frac{\pi x}{a}\right)\;\text{(W/m}^2\text{)}$$

        可见平均功率沿 $+z$ 方向传播，$x$ 方向无净功率传输（驻波分量）。


!!! warning "真题"


    **真题 7.16（来源：2022春季A卷 三、2）**
    **题目：** 矩形波导的尺寸为 $a = 2\;\text{cm}$，$b = 1\;\text{cm}$，内部充满空气。矩形波导中的纵向场瞬时值表示式如下：

    $$H_z(x,y,z,t) = \cos(k_x x)\cos(\omega t - k_z z)\;\text{A/m}$$

    $$E_z(x,y,z,t) = 0\;\text{V/m}$$

    式中，$x, y, z$ 坐标的单位是 cm。$\varepsilon_0 = \dfrac{1}{36\pi}\times10^{-9}\;\text{F/m}$，$\mu_0 = 4\pi\times10^{-7}\;\text{H/m}$。

    求：(1) 波导内电磁场其他全部分量的频域表示式；(2) 求出波导内任意点的平均功率密度；(3) 求出穿过矩形波导内任意横截面的总平均功率。

    ??? note "点击查看解答"

        **解：**

        由于 $E_z = 0$，$H_z \neq 0$，这是 TE 模。由 $H_z = \cos(k_x x)\cos(\omega t - k_z z)$ 可知这是 $\mathrm{TE}_{10}$ 模（$m=1, n=0$），$k_x = \dfrac{\pi}{a}$。

        **(1) 频域表示式**

        复数形式：$\dot{H}_z = \cos(k_x x)\,e^{-jk_z z}$（以余弦为基准）

        由 TE 模横向场与纵向场关系：

        $$\dot{E}_x = 0,\quad \dot{E}_z = 0$$

        $$\dot{E}_y = \frac{j\omega\mu_0}{k_c^2}\frac{\partial\dot{H}_z}{\partial x} = -\frac{j\omega\mu_0 k_x}{k_c^2}\sin(k_x x)\,e^{-jk_z z}$$

        其中 $k_c = k_x = \pi/a$：

        $$\dot{E}_y = -\frac{j\omega\mu_0}{k_x}\sin(k_x x)\,e^{-jk_z z} = -\frac{j\omega\mu_0 a}{\pi}\sin\!\left(\frac{\pi x}{a}\right)e^{-jk_z z}\;\text{(V/m)}$$

        $$\dot{H}_x = \frac{jk_z}{k_c^2}\frac{\partial\dot{H}_z}{\partial x} = -\frac{jk_z}{k_x}\sin(k_x x)\,e^{-jk_z z} = -\frac{jk_z a}{\pi}\sin\!\left(\frac{\pi x}{a}\right)e^{-jk_z z}\;\text{(A/m)}$$

        $$\dot{H}_y = 0$$

        **(2) 平均功率密度**

        $$\mathbf{S}_{\mathrm{av}} = \frac{1}{2}\operatorname{Re}[\dot{\mathbf{E}}\times\dot{\mathbf{H}}^*]$$

        $\dot{\mathbf{E}} = \hat{\mathbf{y}}\,\dot{E}_y$，$\dot{\mathbf{H}}^* = \hat{\mathbf{x}}\,\dot{H}_x^* + \hat{\mathbf{z}}\,\dot{H}_z^*$

        $$\dot{\mathbf{E}}\times\dot{\mathbf{H}}^* = \hat{\mathbf{y}}\times\hat{\mathbf{x}}\,\dot{E}_y\dot{H}_x^* + \hat{\mathbf{y}}\times\hat{\mathbf{z}}\,\dot{E}_y\dot{H}_z^*$$

        $$= -\hat{\mathbf{z}}\,\dot{E}_y\dot{H}_x^* + \hat{\mathbf{x}}\,\dot{E}_y\dot{H}_z^*$$

        $\dot{E}_y\dot{H}_x^* = \left(-\dfrac{j\omega\mu_0}{k_x}\right)\left(-\frac{jk_z}{k_x}\right)\sin^2(k_x x) = -\frac{\omega\mu_0 k_z}{k_x^2}\sin^2(k_x x)$（实数）

        $\dot{E}_y\dot{H}_z^* = -\dfrac{j\omega\mu_0}{k_x}\sin(k_x x)\cos(k_x x)$（纯虚数，取实部为零）

        因此：

        $$\mathbf{S}_{\mathrm{av}} = \hat{\mathbf{z}}\,\frac{\omega\mu_0 k_z}{2k_x^2}\sin^2(k_x x) = \hat{\mathbf{z}}\,\frac{\omega\mu_0 k_z a^2}{2\pi^2}\sin^2\!\left(\frac{\pi x}{a}\right)\;\text{(W/m}^2\text{)}$$

        **(3) 总平均功率**

        $$P = \int_0^a\int_0^b S_{\mathrm{av},z}\,dx\,dy = \frac{\omega\mu_0 k_z a^2 b}{2\pi^2}\int_0^a\sin^2\!\left(\frac{\pi x}{a}\right)dx$$

        $$\int_0^a\sin^2\!\left(\frac{\pi x}{a}\right)dx = \frac{a}{2}$$

        $$P = \frac{\omega\mu_0 k_z a^3 b}{4\pi^2}\;\text{(W)}$$


!!! warning "真题"


    **真题 7.17（来源：2024秋季B卷 三、1）**
    **题目：** 两块无限大平行理想导体板垂直于 $x$ 轴，$0 \leq x \leq d$ 为板间区域为空气，已知其间电场为：

    $$\dot{\mathbf{E}} = \hat{\mathbf{y}}\,E_m\sin\!\left(\frac{2\pi}{d}x\right)e^{-j\beta z}\;\text{(V/m)}$$

    其中 $\beta = \sqrt{k^2 - (2\pi/d)^2}$，$k = \omega\sqrt{\varepsilon_0\mu_0}$。

    (1) 求磁场强度 $\dot{\mathbf{H}}$；(2) 将 $\dot{\mathbf{E}}$ 分解为均匀平面波叠加的形式。
    ...
    ??? note "点击查看解答"
        ...
        **(1) 求磁场强度**
        ...
        由 $\nabla\times\dot{\mathbf{E}} = -j\omega\mu_0\dot{\mathbf{H}}$，电场仅有 $y$ 分量：

        $$(\nabla\times\dot{\mathbf{E}})_x = -\frac{\partial\dot{E}_y}{\partial z} = j\beta E_m\sin\!\left(\frac{2\pi x}{d}\right)e^{-j\beta z}$$

        $$(\nabla\times\dot{\mathbf{E}})_z = \frac{\partial\dot{E}_y}{\partial x} = \frac{2\pi E_m}{d}\cos\!\left(\frac{2\pi x}{d}\right)e^{-j\beta z}$$

        $$\dot{\mathbf{H}} = \frac{j}{\omega\mu_0}\nabla\times\dot{\mathbf{E}} = \hat{\mathbf{x}}\,\frac{-\beta E_m}{\omega\mu_0}\sin\!\left(\frac{2\pi x}{d}\right)e^{-j\beta z} + \hat{\mathbf{z}}\,\frac{j2\pi E_m}{\omega\mu_0 d}\cos\!\left(\frac{2\pi x}{d}\right)e^{-j\beta z}$$

        利用 $\beta/(\omega\mu_0) = \beta/(k\eta_0)$：

        $$\dot{\mathbf{H}} = -\hat{\mathbf{x}}\,\frac{\beta E_m}{k\eta_0}\sin\!\left(\frac{2\pi x}{d}\right)e^{-j\beta z} + \hat{\mathbf{z}}\,\frac{j2\pi E_m}{\omega\mu_0 d}\cos\!\left(\frac{2\pi x}{d}\right)e^{-j\beta z}\;\text{(A/m)}$$

        **(2) 分解为均匀平面波叠加**

        利用欧拉公式：

        $$\sin\!\left(\frac{2\pi x}{d}\right) = \frac{1}{2j}\left(e^{j2\pi x/d} - e^{-j2\pi x/d}\right)$$

        $$\dot{\mathbf{E}} = \hat{\mathbf{y}}\,\frac{E_m}{2j}\left[e^{-j(\beta z - 2\pi x/d)} - e^{-j(\beta z + 2\pi x/d)}\right]\;\text{(V/m)}$$

        即两个均匀平面波的叠加：

        $$\dot{\mathbf{E}} = \dot{\mathbf{E}}_1 + \dot{\mathbf{E}}_2$$

        其中：

        - $\dot{\mathbf{E}}_1 = \hat{\mathbf{y}}\,\dfrac{E_m}{2j}\,e^{-j\mathbf{k}_1\cdot\mathbf{r}}$，波矢量 $\mathbf{k}_1 = -\dfrac{2\pi}{d}\hat{\mathbf{x}} + \beta\hat{\mathbf{z}}$

        - $\dot{\mathbf{E}}_2 = -\hat{\mathbf{y}}\,\dfrac{E_m}{2j}\,e^{-j\mathbf{k}_2\cdot\mathbf{r}}$，波矢量 $\mathbf{k}_2 = \dfrac{2\pi}{d}\hat{\mathbf{x}} + \beta\hat{\mathbf{z}}$

        两波传播方向与 $z$ 轴夹角 $\theta$ 满足 $\cos\theta = \beta/k$，即两波以相同角度从两侧斜入射，叠加形成驻波。


!!! warning "真题"


    **真题 7.18（来源：2024秋季B卷 三、4）**
    **题目：** 空气填充的矩形波导截面尺寸为 $a\times b$（$a > b$），频率为 $f$ 的电磁波在波导中只能以 $\mathrm{TE}_{10}$ 模式传输，其电场强度为：

    $$\dot{\mathbf{E}} = \hat{\mathbf{y}}\sin\!\left(\frac{\pi x}{a}\right)e^{-j\beta z}\;\text{(V/m)}$$

    求：(1) 相移常数 $\beta$；(2) 穿过波导横截面上的平均功率密度矢量；(3) 测量波导波长 $\lambda_g$ 时开槽位置及原因。
    ...
    ??? note "点击查看解答"
        ...
        **(1) 相移常数 $\beta$**
        ...
        $\mathrm{TE}_{10}$ 模截止波数 $k_c = \dfrac{\pi}{a}$，截止频率 $f_c = \dfrac{c}{2a}$。

        自由空间波数 $k = \dfrac{2\pi f}{c} = \dfrac{\omega}{c}$。

        $$\beta = \sqrt{k^2 - k_c^2} = \sqrt{\left(\frac{2\pi f}{c}\right)^2 - \left(\frac{\pi}{a}\right)^2} = \frac{2\pi}{c}\sqrt{f^2 - f_c^2}$$

        或写成：

        $$\beta = k\sqrt{1 - \left(\frac{f_c}{f}\right)^2} = \frac{2\pi}{\lambda}\sqrt{1 - \left(\frac{\lambda}{2a}\right)^2}$$

        **(2) 平均功率密度矢量**

        由 $\nabla\times\dot{\mathbf{E}} = -j\omega\mu_0\dot{\mathbf{H}}$ 可得：

        $$\dot{H}_x = -\frac{\beta}{\omega\mu_0}\sin\!\left(\frac{\pi x}{a}\right)e^{-j\beta z} = -\frac{1}{\eta_0}\sqrt{1-\left(\frac{f_c}{f}\right)^2}\sin\!\left(\frac{\pi x}{a}\right)e^{-j\beta z}$$

        $$\dot{H}_z = \frac{j\pi}{\omega\mu_0 a}\cos\!\left(\frac{\pi x}{a}\right)e^{-j\beta z}$$

        平均功率密度：

        $$\mathbf{S}_{\mathrm{av}} = \frac{1}{2}\operatorname{Re}[\dot{\mathbf{E}}\times\dot{\mathbf{H}}^*] = \hat{\mathbf{z}}\,\frac{1}{2}\operatorname{Re}[-\dot{E}_y\dot{H}_x^*]$$

        $\dot{E}_y\dot{H}_x^* = \sin\!\left(\frac{\pi x}{a}\right)\cdot\left(-\frac{\beta}{\omega\mu_0}\right)\sin\!\left(\frac{\pi x}{a}\right) = -\frac{\beta}{\omega\mu_0}\sin^2\!\left(\frac{\pi x}{a}\right)$（实数）

        $$\mathbf{S}_{\mathrm{av}} = \hat{\mathbf{z}}\,\frac{\beta}{2\omega\mu_0}\sin^2\!\left(\frac{\pi x}{a}\right)\;\text{(W/m}^2\text{)}$$
        ...
        **开槽位置**：应在波导宽边中央（$x = a/2$）沿轴向（$z$ 方向）开槽。

        **原因**：$\mathrm{TE}_{10}$ 模在宽边中央 $x = a/2$ 处，$\sin(\pi x/a) = \sin(\pi/2) = 1$，电场最强但仅有 $y$ 分量（垂直于宽边）。波导壁电流在宽边中央沿 $z$ 方向流动（$J_{sz}$ 分量），开槽方向与电流方向一致，不切断电流路径，对波导内场分布扰动最小。
